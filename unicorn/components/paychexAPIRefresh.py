            ...
            if employee["organization"]["organizationId"] == PAYCHEX_ORG:
                ...
                if emp.count() > 0:
                    ...
                    if emp.paychex_flex_workerID is None or emp.paychex_flex_workerID == "":
                        emp.paychex_flex_workerID = employee["workerId"]
                        emp.save()
                    ...
                    if (employee["currentStatus"]["statusType"] == "ACTIVE"):
                        self.statusMessages.append({"type": "e", "text": f"No employee found for {employee['name']['givenName']} {employee['name']['familyName']}, FlexID: {employee['workerId']}"})
