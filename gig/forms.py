...
class StaffModelChoiceField(forms.ModelChoiceField):
    ...
    def label_from_instance(self, obj):
        return (
            ...
            f"{obj}"
            f'staffing score: {functools.reduce(lambda a, b: a+(1 if Job.objects.filter(employee=obj, gig__pk=b[0]).first() is not None else -1), JobInterest.objects.filter(employee=obj).values_list("job__gig").distinct(), 0)*((timezone.now() - Job.objects.filter(employee=obj).order_by("-gig__start").first().gig.start).days if Job.objects.filter(employee=obj).order_by("-gig__start").first() is not None else ((timezone.now() - JobInterest.objects.filter(employee=obj).values_list("job__gig__start").order_by("job__gig__start").first()[0]).days if JobInterest.objects.filter(employee=obj).count() > 0 else 0))}, '  # Reduce all job interests, add 1 for every gig staffed and -1 for every gig skipped. Multiply by last time staffed to account for multiple jobs on same gig (can't be staffed more than once)
