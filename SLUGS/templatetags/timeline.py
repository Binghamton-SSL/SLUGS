...
def timeline(emp):
    next_job = Job.objects.filter(employee=emp).filter(gig__end__gte=(timezone.now() + timezone.timedelta(hours=-5))).order_by("gig__end")
    if len(next_job) > 0:
        ...
        gig_happening = your_load_ins.first().shop_time <  ( timezone.now() + timezone.timedelta(minutes=10) )  and your_load_ins.last().load_out > ( timezone.now() + timezone.timedelta(hours=-5) )
        ...
        events = [
            {
                ...
                "start": timezone.template_localtime(next_gig.start).strftime("%m/%d/%Y, %H:%M:%S"),
                "end": timezone.template_localtime(next_gig.end).strftime("%m/%d/%Y, %H:%M:%S"),
                "group": len(your_depts)-1,
            },
            {
                "content": f'Setup-by Time <br><sub>{timezone.template_localtime(next_gig.setup_by).strftime("%H:%M")}</sub>',
                "start": timezone.template_localtime(next_gig.setup_by).strftime("%m/%d/%Y, %H:%M:%S"),
                "group": len(your_depts)-1,
            ...
            events.extend([
                {
                    ...
                    "start": timezone.template_localtime(load_in.shop_time).strftime("%m/%d/%Y, %H:%M:%S"),
                    "group": group,
                },
                {
                    "content": f'Load in <br><sub>{timezone.template_localtime(load_in.load_in).strftime("%H:%M")}</sub>',
                    "start": timezone.template_localtime(load_in.load_in).strftime("%m/%d/%Y, %H:%M:%S"),
                    "group": group,
                },
                {
                    "content": f'Load out <br><sub>{timezone.template_localtime(load_in.load_out).strftime("%H:%M")}</sub>',
                    "start": timezone.template_localtime(load_in.load_out).strftime("%m/%d/%Y, %H:%M:%S"),
                    "group": group,
                },
            ])
    return locals()
